...
class FaceBookCrawler:
    ...
    POST_ID_PATTERN = re.compile(r"https?://.*?/.*?/\d+/posts/(\d+)/\?.*",
                                 re.I)
    LOGON_SELECTOR = 'div[class="x1i10hfl x1qjc9v5 xjqpnuy xa49m3k xqeqjp1 ' \
    ...
    IMAGE_IMG_SELECTOR = 'img[class="x1ey2m1c xds687c x5yr21d x10l6tqk ' \
                         'x17qophe x13vifvy xh8yej3"]'
    SINGLE_IMAGE_IMG_SELECTOR = 'img[class="x1ey2m1c xds687c x5yr21d ' \
                                'x10l6tqk x17qophe x13vifvy xh8yej3 xl1xv1r"]'
    LOAD_REPLY_MORE_SELECTOR = 'div[class="x1i10hfl xjbqb8w xjqpnuy xa49m3k ' \
    ...
    LOAD_SEE_MORE_SELECTOR = 'div[class="x1i10hfl xjbqb8w x6umtig x1b1mbwd ' \
    ...
    POST_ID_LOCATE_SELECTOR = 'span[class="x4k7w5x x1h91t0o x1h9r5lt xv2umb2 '\
                              'x1beo9mf xaigb6o x12ejxvf x3igimt xarpa2k ' \
                              'xedcshv x1lytzrv x1t2pt76 x7ja8zs x1qrby5j ' \
                              'x1jfb8zj"]'
    POST_ID_SELECTOR = 'a[class="x1i10hfl xjbqb8w x6umtig x1b1mbwd xaqea5y ' \
    ...
    POST_BODY_SELECTOR = 'span.x193iq5w.xeuugli.x13faqbe.x1vvkbs.xlh3980' \
    ...
    POST_BODY_SELECTOR_REFILL = 'div[class="x1iorvi4 x1pi30zi x1l90r2v ' \
    ...
    POST_BODY_SELECTOR_LOGON_MODE = 'span[class="x193iq5w xeuugli x13faqbe ' \
                                    'x1vvkbs xlh3980 xvmahel x1n0sxbx ' \
                                    'x1lliihq x1s928wv xhkezso x1gmr53x ' \
                                    'x1cpjm7i x1fgarty x1943h6x x4zkp8e ' \
                                    'x3x7a5m x6prxxf xvq8zen xo1l8bm xzsf02u '\
                                    'x1yc453h"]'
    POST_COMMENT_BLOCK_SELECTOR = 'div[class="x168nmei x13lgxp2 x30kzoy ' \
                                  'x9jhf4c x6ikm8r x10wlt62"] > div > div[' \
                                  'class="x1jx94hy x12nagc"]'
    POST_COMMENT_LIST_SELECTOR_L_MODE = 'div[class="x1i10hfl x1qjc9v5 ' \
                                        ...
                                        'xt0b8zv"]'
    POST_COMMENT_MORE_SELECTOR_LOGON_MODE = 'div[class="x168nmei x13lgxp2 ' \
                                            'x30kzoy x9jhf4c x6ikm8r ' \
                                            'x10wlt62"]'
    POST_COMMENT_BLOCK_SELECTOR_LOGON_MODE = 'div[class="x168nmei x13lgxp2 ' \
                                             'x30kzoy x9jhf4c x6ikm8r ' \
                                             'x10wlt62"] > div > div[class=" '\
                                             'xzueoph"]'
    POST_COMMENT_FLOAT_WINDOWS_LOGON_MODE = 'div[class="xwya9rg x11i5rnm ' \
                                            'x1e56ztr x1mh8g0r xh8yej3"]'

    POST_AUTHOR_AREA_SELECTOR = 'span[class="x193iq5w xeuugli x13faqbe ' \
    ...
    def __init__(self, url, userid=None, passwd=None,
        ...
        self.driver = webdriver.Chrome(executable_path=self.exe,
    ...
    def open_and_wait(self):
        ...
        try:
            ...
            WebDriverWait(self.driver
                          , timeout=self.TIMEOUT).until(post_element)

        except TimeoutException:
            logger.error('Over Time ! Check Network Condition')
    ...
    def login(self):
        ...
        self.driver.find_element(
            By.CSS_SELECTOR, self.LOGON_SELECTOR
        ).click()
        try:
            post_element = EC.presence_of_element_located(
                (By.CSS_SELECTOR, self.POST_BLOCK_SELECTOR)
            ...
            WebDriverWait(self.driver,
                          timeout=self.LOGON_TIME).until(post_element)
        except TimeoutException:
            logger.error('Over Time ! Check Network Condition')
    ...
    def grab_image(self, element):
        ...
        :return: Generator of Every Post Image
        """
        ...
        for i in images_list:
            image_pocket.append(
                i.find_element(By.CSS_SELECTOR,
                               self.IMAGE_IMG_SELECTOR).get_attribute('src')
            )

        # Check Single Image
        images_list = element.find_elements(By.CSS_SELECTOR,
                                            self.SINGLE_IMAGE_IMG_SELECTOR)
        for i in images_list:
    ...
    def click_load_comment(self, element):
        ...
        """
        if not self.logon_mode:
            if len(element.find_elements(
                    By.CSS_SELECTOR, self.POST_COMMENT_BLOCK_SELECTOR)) > 0:
                comment_area = element.find_element(
                    By.CSS_SELECTOR, self.POST_COMMENT_BLOCK_SELECTOR
                )
            else:
                logger.warning('No Comment Area')
                return None, None
            ...
            # Logon Mode
            if len(element.find_elements(By.CSS_SELECTOR, self.POST_COMMENT_LIST_SELECTOR_L_MODE)) > 0:
                float_button = element.find_elements(By.CSS_SELECTOR,
                                                     self.POST_COMMENT_LIST_SELECTOR_L_MODE)[0]
                # First Click To Load Comment
                self._click(float_button)
                _float_windows = EC.presence_of_element_located((By.CSS_SELECTOR,
                                                                 self.POST_COMMENT_FLOAT_WINDOWS_LOGON_MODE))
                try:
                    WebDriverWait(self.driver, timeout=self.LOGON_TIME).until(_float_windows)
                except TimeoutException:
                    self._click(float_button)
                # Float Window will out of comment block
                # Re-locate
                float_windows = self.driver.find_elements(By.CSS_SELECTOR, self.POST_COMMENT_FLOAT_WINDOWS_LOGON_MODE)
                if len(float_windows) > 0:

                    float_window = float_windows[0]
                    for i in range(self.COMMENT_DEEP_COUNT):
                        for button in float_window.find_elements(By.CSS_SELECTOR, self.LOAD_REPLY_MORE_SELECTOR):
                            if '隱藏' in button.text or 'Hide' in button.text:
                                continue
                            self._click(button, no_wait=True)
                        time.sleep(self.SCROLL_PAUSE_TIME)
                    ...
                    logger.warning('Float Windows Not Appear !')
                    if len(element.find_elements(By.CSS_SELECTOR, self.POST_COMMENT_BLOCK_SELECTOR)) > 0:
                        comment_area = element.find_element(By.CSS_SELECTOR, self.POST_COMMENT_BLOCK_SELECTOR)
                    else:
                        logger.warning('No Comment Area')
                        return None, None
                    ...
                    for i in range(self.COMMENT_DEEP_COUNT):
                        ...
                        for button in comment_area.find_elements(By.CSS_SELECTOR, self.LOAD_REPLY_MORE_SELECTOR):
                            if '隱藏' in button.text or 'Hide' in button.text:
                                continue
                ...
                logger.debug('No Comment To Grab')
                return None, None
    ...
    def click_see_more(self, element):
        ...
        """
        logger.debug(element.get_attribute('innerHTML'))
        try:
            button = element.find_element(By.CSS_SELECTOR, self.LOAD_SEE_MORE_SELECTOR)
            self._click(button)
        except NoSuchElementException:
            logger.debug('No See More')
            pass
    ...
    def grab_comment(self, element):
        ...
        comment_pocket = []
        logger.debug("Grabing every Comment")
        if not comment_area:
            return comment_pocket
        for li in comment_area.find_elements(By.CSS_SELECTOR, self.POST_BODY_SELECTOR):
            comment_pocket.append(li.text)
    ...
    def grab_post(self):
        ...
        for i, every_post in enumerate(self.driver.find_elements(By.CSS_SELECTOR, self.POST_BLOCK_SELECTOR)):
            if self.POST_COUNTER > i:
                continue
            else:
                self.POST_COUNTER += 1

            logger.info("===========  POST %d  ================" % i)

            ActionChains(self.driver).scroll_to_element(every_post).perform()
            time.sleep(self.SCROLL_PAUSE_TIME)

            # Check Post ID
            if len(every_post.find_elements(By.CSS_SELECTOR, self.POST_ID_SELECTOR)) > 0:
                try:
                    ec = EC.element_to_be_clickable((By.CSS_SELECTOR, self.POST_ID_LOCATE_SELECTOR))
                    WebDriverWait(self.driver, timeout=self.WAIT_ACTIVE_TIME).until(ec)
                    ActionChains(self.driver).move_to_element(
                        every_post.find_elements(By.CSS_SELECTOR, self.POST_ID_LOCATE_SELECTOR)[0]
                    ).perform()
                except TimeoutException:
                    logger.error('Moving To get Post ID fail')
                    pass
                rw_postid = every_post.find_elements(By.CSS_SELECTOR, self.POST_ID_SELECTOR)[0].get_attribute('href')
                if re.match(self.POST_ID_PATTERN, rw_postid):
                    postid = re.match(self.POST_ID_PATTERN, rw_postid).group(1)
                else:
                    logger.warning('No PostID Found ! Row postID is %s' % rw_postid)
                    postid = '{} - {}'.format(i, time.time())
            else:
                logger.error('Cant Select postID, Check POST %d' % i)
                continue

            if self.logon_mode and len(every_post.find_elements(By.CSS_SELECTOR, self.POST_BODY_SELECTOR_LOGON_MODE)) \
                    > 0:
                text_body = every_post.find_element(By.CSS_SELECTOR, self.POST_BODY_SELECTOR_LOGON_MODE)
            elif len(every_post.find_elements(By.CSS_SELECTOR, self.POST_BODY_SELECTOR_LOGON_MODE)) > 0:
                text_body = every_post.find_element(By.CSS_SELECTOR, self.POST_BODY_SELECTOR_LOGON_MODE)
            elif len(every_post.find_elements(By.CSS_SELECTOR, self.POST_BODY_SELECTOR)) > 0:
                text_body = every_post.find_element(By.CSS_SELECTOR, self.POST_BODY_SELECTOR)
            else:
                logger.warning('Post has not body : %s' % postid)
                continue

            if not str.strip(text_body.text):
                logger.warning('Refill Post Body')
                text_body = every_post.find_element(By.CSS_SELECTOR, self.POST_BODY_SELECTOR_REFILL)
                logger.debug(text_body.get_attribute('innerHTML'))
                self.click_see_more(text_body)
            else:
                self.click_see_more(text_body)
            logger.debug('POST ID is %s' % postid)
            # Body

            _author = every_post.find_element(By.CSS_SELECTOR, self.POST_AUTHOR_AREA_SELECTOR)
            author = _author.find_element(By.CSS_SELECTOR, self.POST_AUTHOR_TEXT_SELECTOR).text
            logger.debug('Author is %s' % author)
            content = text_body.text
            logger.debug('Body text %s' % text_body.text)

            image_pocket = self.grab_image(every_post)
            logger.debug('Image List : {}'.format(image_pocket))

            # Comment Area
            comment_pocket = self.grab_comment(every_post)
            logger.debug('Comment List : {}'.format(comment_pocket))

            logger.debug('Writing To Output')
            self.output_post(postid=postid, author=author, content=content,
                             img_list=image_pocket, comment_list=comment_pocket)
    ...
    def start(self):
        ...
        logger.debug('Recording Page Height : {}'.format(last_height))
        for i in range(3):
            # First Scroll
            self.scroll_down()
